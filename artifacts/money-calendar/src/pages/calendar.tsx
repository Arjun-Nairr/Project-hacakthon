    ...
    <BayzatiMobileShell active="calendar" floatingAction={<button onClick={() => setEntryOpen(true)} className="flex min-h-11 items-center gap-2 rounded-full bg-[#D20A58] px-4 py-3 text-[12px] font-semibold text-white shadow-lg" data-testid="button-add-calendar-event"><Plus className="size-4" /> Add to calendar</button>}>
      <div data-testid="page-calendar">
        ...
        <section className="mt-5 overflow-hidden rounded-[18px] border border-[#E4E7EC] bg-white p-3" data-testid="calendar-month-grid">
          ...
          <div className="grid grid-cols-7 gap-1 text-center">{['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, index) => <span key={`${day}-${index}`} className="py-2 text-[10px] font-semibold text-[#98A2B3]">{day}</span>)}{Array.from({ length: firstWeekday }, (_, index) => <span key={`empty-${index}`} />)}{Array.from({ length: days }, (_, index) => { const day = index + 1; const dayEvents = events.filter((event) => event.day === day); return <button type="button" key={day} onClick={() => setSelectedDay(day)} className={`relative min-h-[54px] rounded-xl border p-1.5 text-left transition-colors ${selectedDay === day ? 'border-[#139BE8] bg-[#EAF6FD]' : 'border-transparent hover:border-[#E4E7EC]'}`} data-testid={`button-calendar-day-${day}`}><span className={`text-[12px] font-semibold ${selectedDay === day ? 'text-[#003B73]' : 'text-[#667085]'}`}>{day}</span>{dayEvents.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{dayEvents.slice(0, 3).map((event) => <span key={event.id} className="size-1.5 rounded-full" style={{ backgroundColor: eventTone(event).dot }} />)}</div>}{dayEvents.length > 3 && <span className="absolute bottom-1 right-1 text-[8px] font-bold text-[#98A2B3]">+{dayEvents.length - 3}</span>}</button>; })}</div>
