...
function ProgressRing({ value, tone }: { value: number; tone: Goal['tint'] }) {
  ...
  return (
    <div className={`relative grid size-[76px] shrink-0 place-items-center rounded-full ${tone === 'berry' ? 'bg-[#FBEAF2]' : tone === 'blue' ? 'bg-[#E8F0F8]' : 'bg-[#E8F7FD]'}`} data-testid={`progress-ring-${tone}`}>
      ...
      <span className={`relative font-mono-data text-sm font-bold ${toneClass}`}>{value}%</span>
    </div>
  );
}
...
function GoalCard({ goal, onAdd }: { goal: Goal; onAdd: (goal: Goal) => void }) {
  ...
  return (
    <article className="soft-lift reveal rounded-[24px] border border-[#DCE8EE] bg-[#FBFEFF] p-5 shadow-[0_8px_28px_rgba(0,59,115,.04)]" data-testid={`card-goal-${goal.id}`}>
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <div className="mb-3 flex items-center gap-2">
            <span className={`size-2 rounded-full ${goal.tint === 'berry' ? 'bg-[#D20A58]' : goal.tint === 'blue' ? 'bg-[#003B73]' : 'bg-[#139BE8]'}`} />
            <span className="font-mono-data text-[10px] uppercase tracking-[.16em] text-[#667085]">Savings goal</span>
          </div>
          <h2 className="font-display text-[29px] leading-[.95] text-[#003B73]" data-testid={`text-goal-name-${goal.id}`}>{goal.name}</h2>
      ...
      <div className="mt-4 flex items-center justify-between gap-3">
        ...
        <button onClick={() => onAdd(goal)} className="flex min-h-10 items-center gap-1.5 rounded-full bg-[#EAF6FD] px-3 text-[11px] font-bold text-[#003B73] transition-transform hover:-translate-y-0.5" data-testid={`button-add-to-goal-${goal.id}`}><Plus className="size-3.5" /> Add money</button>
      </div>
    </article>
  );
}
...
export default function GoalsPage() {
  ...
  return (
    <div className="relative" data-testid="page-goals">
      <div className="pointer-events-none absolute -right-24 -top-28 size-72 rounded-full bg-[#DDF3FB] opacity-70" />
      <div className="relative">
        <header className="reveal flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="font-mono-data text-[10px] uppercase tracking-[.2em] text-[#D20A58]">Your next yes</p>
            <h1 className="mt-2 max-w-[630px] font-display text-[52px] leading-[.9] tracking-tight text-[#003B73] md:text-[68px]" data-testid="heading-goals">Small steps, visible future.</h1>
            <p className="mt-4 max-w-[560px] text-[14px] leading-6 text-[#667085]">Goals turn a vague wish into a number that can sit gently inside your month. No pressure, just a place to notice the progress.</p>
          </div>
          <button onClick={() => setNewGoalOpen(true)} className="flex min-h-12 items-center justify-center gap-2 self-start rounded-full bg-[#D20A58] px-5 text-[12px] font-bold text-white shadow-[0_8px_20px_rgba(210,10,88,.18)] transition-transform hover:-translate-y-0.5 md:self-end" data-testid="button-create-goal"><Plus className="size-4" /> New goal</button>
        </header>

        <section className="reveal reveal-delay-1 mt-8 grid gap-3 md:grid-cols-[1.35fr_.65fr]">
          <div className="relative overflow-hidden rounded-[26px] bg-[#003B73] p-6 text-[#F2FBFD] md:p-7" data-testid="card-goals-overview">
            <div className="absolute -right-8 -top-12 size-40 rounded-full border-[18px] border-[#139BE8]/25" />
            <div className="relative">
              <p className="flex items-center gap-2 font-mono-data text-[10px] uppercase tracking-[.16em] text-[#9BE4F5]"><Target className="size-4" /> Across your goals</p>
              <p className="mt-4 font-display text-[45px] leading-none" data-testid="text-total-saved">{formatAed(totalSaved)}</p>
          ...
          <div className="rounded-[26px] border border-[#DCE8EE] bg-[#F6FBFD] p-6" data-testid="card-goals-note">
            <CircleHelp className="size-5 text-[#139BE8]" />
            <h2 className="mt-7 font-display text-[27px] leading-none text-[#003B73]">A goal is a kindness to future you.</h2>
        ...
        <section className="mt-9" data-testid="section-goal-list">
          <div className="mb-4 flex items-center justify-between"><div><p className="font-mono-data text-[10px] uppercase tracking-[.18em] text-[#98A2B3]">In progress</p><h2 className="mt-1 font-display text-[31px] leading-none text-[#003B73]">Your goals</h2></div><span className="rounded-full bg-[#EAF6FD] px-3 py-1.5 font-mono-data text-[10px] text-[#003B73]" data-testid="text-goal-count">{goals.length} active</span></div>
          <div className="grid gap-4 lg:grid-cols-2">{goals.map((goal) => <GoalCard key={goal.id} goal={goal} onAdd={setSelectedGoal} />)}</div>
        </section>
      </div>
      ...
      {selectedGoal && <div className="fixed inset-0 z-50 bg-[#003B73]/35 p-4 backdrop-blur-[2px]" onClick={() => setSelectedGoal(null)}>
        <section role="dialog" aria-modal="true" aria-labelledby="add-goal-title" onClick={(event) => event.stopPropagation()} className="absolute inset-x-4 bottom-4 mx-auto max-w-[440px] rounded-[26px] bg-[#FBFEFF] p-6 shadow-2xl md:bottom-1/2 md:translate-y-1/2" data-testid="dialog-add-goal">
          ...
          <p className="font-mono-data text-[10px] uppercase tracking-[.16em] text-[#D20A58]">Add to goal</p><h2 id="add-goal-title" className="mt-2 font-display text-[33px] leading-none text-[#003B73]">{selectedGoal.name}</h2><p className="mt-2 text-[12px] text-[#667085]">Current balance {formatAed(selectedGoal.saved)} · target {formatAed(selectedGoal.target)}</p>
          <label className="mt-6 block text-[11px] font-bold text-[#667085]">How much would you like to add?<div className="mt-2 flex items-center rounded-xl border border-[#C9DDE6] bg-white px-3"><span className="font-mono-data text-sm text-[#98A2B3]">AED</span><input type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0" className="w-full bg-transparent px-3 py-3 text-[16px] font-bold text-[#003B73] outline-none" data-testid="input-goal-amount" /></div></label>
      ...
      {newGoalOpen && <div className="fixed inset-0 z-50 bg-[#003B73]/35 p-4 backdrop-blur-[2px]" onClick={() => setNewGoalOpen(false)}>
        <section role="dialog" aria-modal="true" aria-labelledby="new-goal-title" onClick={(event) => event.stopPropagation()} className="absolute inset-x-4 bottom-4 mx-auto max-w-[500px] rounded-[26px] bg-[#FBFEFF] p-6 shadow-2xl md:bottom-1/2 md:translate-y-1/2" data-testid="dialog-create-goal">
          ...
          <p className="font-mono-data text-[10px] uppercase tracking-[.16em] text-[#D20A58]">Make it concrete</p><h2 id="new-goal-title" className="mt-2 font-display text-[33px] leading-none text-[#003B73]">Name a new goal</h2>
