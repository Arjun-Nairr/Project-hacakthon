...
import { ArrowRight, Check, CircleAlert, Home as HomeIcon, Info, Loader2, Sparkles } from 'lucide-react';
...
export default function HomePage() {
  ...
  const sendToCalendar = () => { window.sessionStorage.setItem('money-calendar-home-plan', result?.headline ?? 'Home decision'); setLocation('/'); };
  ...
  return <BayzatiMobileShell active="home">
    <div data-testid="page-home">
      <header className="mt-7">
        <p className="text-[11px] font-semibold uppercase tracking-[.16em] text-[#667085]">Home decision</p>
      ...
      {result ? <section className="mt-4 overflow-hidden rounded-[18px] bg-[#003B73] p-5 text-white" data-testid="section-home-result">
        ...
        <button onClick={sendToCalendar} className="mt-4 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-white text-[12px] font-semibold text-[#003B73]" data-testid="button-send-home-to-calendar"><Check className="size-4" />Hold this commitment in my calendar</button>
        <p className="mt-2 text-center text-[10px] text-[#C4E5EF]">Saved for review; this does not change your calendar.</p>
