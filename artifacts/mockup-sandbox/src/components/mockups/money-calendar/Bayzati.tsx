...
export function Bayzati() {
  ...
  return (
    <main className="relative h-[844px] w-full overflow-hidden bg-[#f6f8fb] font-['DM_Sans'] text-[#173a62]" style={{ maxWidth: 390 }}>
      ...
      <div className="relative px-5 pb-[86px] pt-7">
      ...
      <button onClick={() => setSheetOpen(true)} className="absolute bottom-[74px] right-5 z-20 flex items-center gap-2 rounded-full bg-[#d94c84] px-4 py-3 text-[12px] font-semibold text-white shadow-[0_8px_18px_rgba(217,76,132,.28)] transition-transform active:scale-95"><Plus size={17} /> Add to calendar</button>
      <nav className="absolute inset-x-0 bottom-0 flex h-[66px] items-center justify-around border-t border-[#e3eaf1] bg-white/95 px-3 backdrop-blur-md">
        <button className="flex min-w-[72px] flex-col items-center gap-1 rounded-xl py-1.5 text-[#d94c84]"><CalendarDays size={18} /><span className="text-[10px] font-semibold">Calendar</span></button>
        <button className="flex min-w-[72px] flex-col items-center gap-1 rounded-xl py-1.5 text-[#8a99aa]"><Landmark size={18} /><span className="text-[10px] font-semibold">Borrow</span></button>
        <button className="flex min-w-[72px] flex-col items-center gap-1 rounded-xl py-1.5 text-[#8a99aa]"><Home size={18} /><span className="text-[10px] font-semibold">Home</span></button>
      </nav>
